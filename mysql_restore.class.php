...
class MySQL_Restore
{
    ...
    public function Execute($param, $mode = MSR_STRING, $is_compressed = false, $split_only = false)
    {
        if ($mode == MSR_FILE) {
    ...
    public function _Decompose(&$ret, $sql)
    {
        $sql = rtrim($sql, "\n\r");
        $sql_len = strlen($sql);
        $char = '';
    ...
    public function _Restore($sql, $split_only)
    {
        if (!$this->_Connect()) {
            return false;
        }
    ...
    public function _ReadFromFile($fname, $is_compressed)
    {
        if ($is_compressed) {
            $sql = gzfile($fname);
        } else {
